...
def test_costbasis_receive_unknown_token_with_no_price_creates_flagged_zerocost_lot(
    test_db,
):
    ...
    flags = lot.flags
    assert "zero_price" in [f["name"] for f in lot.flags], f"No zero_price in lot {lot}"


def test_costbasis_get_change_back_from_deposit_same_token_in_change(test_db):
...
def test_costbasis_non_disposal_lots_have_history_ref_to_original_asset(test_db):
...
def test_costbasis_get_change_back_from_deposit_wrapped_token_in_change(test_db):
    make.tx(ins=["50 AVAX"], timestamp=1, from_address="A Friend")
    price_feed.stub_price(1, "avalanche-2", 1.00)

    # We need to use the actual WAVAX token contract since this is how our code will match for wrapped tokens
    make.tx(
        ins=["1 WAVAX|0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7", "10 mooAVAX|0xMoo"],
        outs=["10 AVAX"],
        debank_name="deposit",
        timestamp=2,
        from_address="Beefy",
...
def test_costbasis_lot_generated_simplest_case(test_db):
...
def test_lot_and_disposal_use_tx_ledger_price_if_set(test_db, event_store):
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
    ...
    tx = tx_ledgers[0]
    event = event_store.create_tx_ledger_price_updated(tx.id, 20.00, source="manual")
    ...
    assert tx.amount == 1
    event = event_store.create_tx_ledger_price_updated(tx.id, 10.00, source="manual")
    event_store.apply_event(event)
...
def test_respects_ignored_from_costbasis_flag_on_tx_logicals(test_db, event_store):
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
...
def test_receive_flags_costbasis_for_disposal(test_db):
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
...
def test_simple_sends_are_not_treated_as_disposals(test_db):
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
...
def test_wrapped_balance_is_correct(test_db):
    ...
    # Wrap
    make.tx(ins=[f"1 WAVAX|{WAVAX}"], outs=["1 AVAX"], timestamp=2, to_address=WAVAX)
    price_feed.stub_price(2, "avalanche-2", 1.00)
...
def test_dispose_of_wrapped_asset(test_db):
    WAVAX = "0xb31f66aa3c1e785363f0875a1b74e27b85fd66c7"
    ...
    assert avax_disposals[0].total_usd == approx(Decimal(2.0))  # price when disposed of
...
def pending_test_send_to_address_not_belonging_to_entity_flags_it_for_review(test_db):
...
def test_costbasis_creates_zero_cost_lot_if_needed_to_auto_reconcile(test_db):
...
def test_costbasis_does_not_dispose_for_deposits(test_db):
    # 5 AVAX in @ 1.00 - makes an initial lot w/ amount 5 for AVAX
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
...
def test_costbasis_maps_like_kind_assets_like_avwavax_to_avax(test_db):
    # 5 AVAX in @ 1.00 - makes an initial lot w/ amount 5 for AVAX
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
...
def test_costbasis_deposit_and_withdrawal_earns_interest(test_db):
    ...
    assert income.net_usd == approx(Decimal(0.60))  # AVAX @ 6.00 * .1 new AVAX = 0.6
...
def test_costbasis_borrow(test_db):
...
def test_costbasis_borrow_and_repay_loan_w_interest(test_db):
    # Initial Receive so we have a non-zero costbasis_lot of AVAX
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
...
def test_costbasis_lp_exit_handles_with_IL(test_db):
...
def test_costbasis_borrow_and_lp(test_db):
    # Borrow 1 AVAX and get 1.2 variableDebtWAVAX
    # value of AVAX at $5
    make.tx(
        ins=["1 AVAX", "1.2 variableDebtWAVAX|0xvariableDebtWAVAX"],
...
def test_costbasis_disposing_like_kind_assets_from_different_asset_tx_ids_results_in_sane_disposals(
    test_db,
):
    make.tx(
        ins=[
...
def test_costbasis_changing_ownership_of_deposit_receipt(test_db):
    # Initial Receive so we have a non-zero costbasis_lot of AVAX
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
...
def pending_test_costbasis_changing_ownership_overdraw_of_deposit_receipt(test_db):
    # Initial Receive so we have a non-zero costbasis_lot of AVAX
    make.tx(ins=["1 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
...
def test_costbasis_disposal_of_deposit_receipt(test_db):
    # Initial Receive so we have a non-zero costbasis_lot of AVAX
    make.tx(ins=["5 AVAX"], timestamp=1, from_address="A FRIEND")
    price_feed.stub_price(1, "avalanche-2", 1.00)
